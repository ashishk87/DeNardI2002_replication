...
function params_dict(;tlength=5, dage=5, dy = 4, def = 45, TL = 9,
                      ...
                      β = (0.9455)^tlength, σ = 1.5, ϕ₁ = -9.5,ϕ₂ = 8.0, γ = 0.83, hbet = 0.67,
                      τₐ = 0.2, τ_b = 0.1, exb = 8.5, ggovt = 0.18, pens = 0.4,
                      maxa = 50.0, mina = 0.0, da = 120, minc = 0.0, maxc = 1.0,
                      σ_z = sqrt(0.412), σ_h = sqrt(0.42), y = [0.2594, 0.6513, 1.5355, 3.8547],
    ...
    y: 4-state productivity grid
    Qy: transition matrix for y
    Qyh: inheritance transition matrices
    ...
    # (Revisit Comment: Gini index for productivity and age-egfficiency adjusted is not computed for now. It can be added later)
    ...
    ϕ = (  (1 .+ (agrid .- τ_b.*max.(agrid .- exb, zeros(size(agrid))))./ ϕ₂).^(1 - σ)   .- 1).*ϕ₁ 
...
# Main function that solves and simulates the model
...
function denardi_main(params)
    ...
    # Unpacking parameters all at once
    ...
    - initialization: they do not expect to receive any bequests. So, all agents are at the asset grid point 1 which is zero
    =#
    ...
    Value function and policy fn for working people:
        ...
        -state4: age
    =#
    vt = zeros(da, dy, (dy+1), TL-3)
    polt = zeros(da, dy, (dy+1), TL-3)
    w_assets = zeros(da, dy, (dy+1), TL-3)
    polc = zeros(da, dy, (dy+1), TL-3)
    ...
    # # Obtain the indices of the asset choice of retirees. This will be used to fill the transition matrix for retirees.
    # indexr = zeros(da, TR)
    # for j in 1:TR
    #     indexr[:,j] = [searchsortedfirst(agrid, val) for val in polr[:, j]]
    # end
    ...
    """
    Up until here function working mechanically correctly, i.e. no obvious errors in the code.
    """

    #=
    We also need to fill in the transition matrix for the oldies who are going to die for sure next period.
    We do that below.
    =#
    ...
    - Working people can receive bequests after they turn 40 so they form expectations of bequests and inheritance. So, we'll have to keep track of the entire bequest distribution.
    ...
    while ϵ₂ > 1e-4 # loop until govt. budget constraint is satisfied
        ...
        while ϵ₁ > 1e-4 # loop until convergence of bequest distribution
            ...
            Case-1: Workers age TL (i.e. age 60). They are going to retire next period.
                    Currently the state space is (cash at hand, productivity, parents productivity).
            ...
            Case-2: Workers aged 55 to 35.
                - They receive income shocks, so far that was not the case
            ...
            open("time_log.txt", "a") do file
                # Write the values to the file
                write(file, "val fun computed at \n$IHOUR $IMIN $ISEC\n")
            end
            ...
            while err > 1e-8
                ...
                for i in 1:nonzeroM
                    invm[colM[i]] += (valM[i] * invm1[rowM[i]])/gpop 
                end
            ...
            for i in 1:dy # loop over agents income at 40
                for j in 1:dy+1 # loop over grandparents income at 40 (might be dead)
                    for k in 1:da # loop over agents assets at 40
                        temprealv4[i] += temprealv2[(j-1)*(dy)*da + (i-1)*da + k]
                    end
                end
            end
        ...
        open("time_log.txt", "a") do file
            # Write the values to the file
            write(file, "beqdist computed at \n$IHOUR $IMIN $ISEC\n")
        end
        ...
        aveinc, numret = age_prod_inv_dist(params)
        ...
        open("time_log.txt", "a") do file
            # Write the values to the file
            write(file, "New  Tau_l computed at \n$IHOUR $IMIN $ISEC\n")
        end
